...
export function SpartanOpsConsole({ fieldId, password }: { fieldId: string; password: string }) {
  ...
  useEffect(() => {
    let alive = true;
    const load = async () => {
      const { data } = await supabase.from("spartanops_captures").select("id, point_number, team, player_callsign, captured_at").eq("field_id", fieldId).eq("suspicious", false).order("captured_at", { ascending: false }).limit(50);
  ...
  const startMatch = async () => {
    ...
    await callPatch({
      ...
      gamemode,
      ...(state?.compressed_map_url ? {} : { compressed_map_url: mapUrl }),
  ...
  const mainLabel = isActive ? (en ? "PAUSE MATCH" : "PAVZIRAJ TEKMO") : isPaused ? (en ? "RESUME MATCH" : "NADALJUJ TEKMO") : (en ? "START MATCH" : "ZAČNI TEKMO");
  const mainColor = isActive ? "#f5b041" : "#9eff3d";

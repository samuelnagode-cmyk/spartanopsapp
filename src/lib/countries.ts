/**
 * ISO 3166-1 country list with emoji flag support.
 *
 * Flags are derived from the two-letter code via regional indicator symbols,
 * so no image assets are needed. `flagFor` also resolves free-text country
 * values stored on legacy missions (e.g. "Deutschland", "England").
 */
...
/** Free-text spellings that existing missions already use. */
...
/** Resolve a stored country string (name, alias, or ISO code) to its ISO code. */
export function countryCodeFor(value?: string | null): string | null {
...
/** Emoji flag for a stored country string, or "" when unknown. */
...
/** Case-insensitive substring search across the country list. */
